...
class ContrastFilter(Filter):
    def __init__(self, sourceImage: Image.Image, c = 8):
        self.c = c
    ...
    def calculateNewPixelColor(self, sourceImage: Image.Image, x: int, y: int):
        ...
        resultColor = (self.Clamp(self.l_avg + (sourceClolor[0] - self.l_avg) * self.c, 0, 255),
                       self.Clamp(self.l_avg + (sourceClolor[1] - self.l_avg) * self.c, 0, 255),
                       self.Clamp(self.l_avg + (sourceClolor[2] - self.l_avg) * self.c, 0, 255))
        return resultColor
